...
export const resetPassword = async (req, res, next) => {
    try {
        const {body: {email, username, token}} = req;
        const hash = await bcrypt.hash(token, 12);
        const user = await User.findOneAndUpdate({$and: [{username, email}]}, {$set: {password: hash}});    
        res.status(http.OK).json({success: true, message: "비밀번호가 리셋되었습니다.", data: {user, token}})
    } catch (e) {
        console.error(e);
        next(e);
    }
...
export const login = async (req, res, next) => {
    passport.authenticate('local', {session: false}, (err, user, info ) => {
        ...
        req.login(user, {session: false}, err => {
            ...
            return res.status(http.OK).json({success:true, message:"로그인 되었습니다.", data: {user, token}});
        })
    })(req, res, next);
}
